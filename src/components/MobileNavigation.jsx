...
function MobileNavigationDialog({ isOpen, close }) {
  ...
  return (
    ...
    >
      <DialogBackdrop
        transition
        className="fixed inset-0 top-14 bg-neutral-400/20 backdrop-blur-sm data-[closed]:opacity-0 data-[enter]:duration-300 data-[leave]:duration-200 data-[enter]:ease-out data-[leave]:ease-in dark:bg-black/40"
      />
...
export function MobileNavigation() {
  ...
  return (
    <IsInsideMobileNavigationContext.Provider value={true}>
      <button
        type="button"
        className="flex h-6 w-6 items-center justify-center rounded-md transition hover:bg-neutral-900/5 dark:hover:bg-white/5"
